...
var EXAMS = [
  {
    ...
    notes: {
      '社會工作': '總論：專業發展史、理論視角、價值與倫理、臺灣社工制度脈絡。',
  ...
  },
  {
    ...
    /* 申論選試科目(第二試):考生於下列 4 科中選 1 科應試,其餘 6 科為共同科目。
       與教師檢定的 subjectGroupSep 不同——教檢的類科會換掉你要考的整組「選擇題」科目,
       律師第一試那 4 卷卻是全員同一份,真正有選擇的只有第二試這一科。故獨立一個欄位,
       只作用在申論分頁;引擎對任何設了 essayElectives 的考試都通用,不寫死律師。
       options 必須列齊官方全部選項:只列部分等於騙人(考該科的人會誤以為站上沒收錄)。 */
    essayElectives: {
      label: '選試科目',
      note: '律師第二試除 6 科共同科目外，另於下列 4 科選 1 科應試。選定後申論題只列你要考的那一科；未選則全部列出。',
      options: ['智慧財產法', '勞動社會法', '財稅法', '海商法與海洋法']
    }
  },
  {
  ...
  {
    key: 'pharmacist', prefix: 'pha_', name: '藥師', short: '藥師', hasEssay: false,
    authority: '考選部', jurisdiction: '中華民國法規',
    sourceName: '考選部「國家考試試題及測驗式試題答案」開放資料',
    category: '醫護', count: 13050,
    blurb: '藥理與藥化、藥物分析與生藥、藥劑與生物藥劑、調劑與臨床藥學、藥物治療、藥事法規 —— 六科純選擇題，涵蓋 101～115 年歷屆。',
    subjects: ['藥理學與藥物化學', '藥物分析與生藥學', '藥劑學與生物藥劑學',
      '調劑學與臨床藥學', '藥物治療學', '藥事行政與法規'],
    notes: {
      '藥理學與藥物化學': '藥物的作用機轉、藥效與副作用，以及藥物結構與活性的關係（構效關係、代謝與化學修飾）。',
      '藥物分析與生藥學': '層析、光譜、質譜等分析方法與含量測定，以及生藥、天然物成分與中藥學。',
      '藥劑學與生物藥劑學': '劑型設計與製劑技術，以及吸收、分布、代謝、排除與藥物動力學參數的計算。',
      '調劑學與臨床藥學': '處方調劑、給藥計算、藥事照護與用藥諮詢，以及臨床上的藥物選擇與監測。',
      '藥物治療學': '各系統疾病的藥物治療策略、療效評估與不良反應處理，以現行臨床指引為準。',
      '藥事行政與法規': '藥事法、藥品管理與查驗登記、管制藥品與藥師執業規範，以現行法規為準。'
    },
  ...
  },
  {
    ...
    /* 科目為「類科・科目」二維:模擬考先選類科(幼兒園/特教身障/特教資優/國小/中等)
       ...
       subjectGroupLabels:類科顯示名覆寫,一律比照教檢官方分類全名(身心障礙組/資賦優異組
       等);內部 group key 仍是 subject 前綴(幼兒園/特教身障…),覆寫只動顯示、不動篩選。 */
    subjectGroupSep: '・',
    subjectGroupLabels: {
      '幼兒園': '幼兒園師資類科', '特教身障': '身心障礙組', '特教資優': '資賦優異組',
      '國小': '國民小學師資類科', '中等': '中等學校師資類科（國中・高中/職）'
    },
    authority: '教育部',
    ...
    blurb: '幼兒園師資類科／身心障礙組／資賦優異組／國民小學師資類科／中等學校師資類科五類科 —— 110～115 年素養導向新制的選擇題，涵蓋國語文、學習者發展、教育理念、課程教學（國小另含數學）。',
    coverageNote: '本題庫僅收錄各卷「選擇題」；教師檢定部分科目另有綜合題／問答題未收錄。科目以「類科・科目」標示，依你報考的類科（比照教檢官方分類）篩選，只出該類科的科目。',
    subjects: [
  ...
  {
    key: 'ast', prefix: 'ast_', name: '分科測驗', short: '分科',
    authority: '大學入學考試中心', jurisdiction: '中華民國',
    sourceName: '大考中心 分科測驗歷年試題（依著作權法 §9，考試試題不受著作權保護）',
    sourceLicense: '大考中心公開試題・著作權法 §9 法定豁免',
    category: '升學', count: 981,
    blurb: '數甲、數乙、物理、化學、生物、歷史、地理、公民與社會 —— 111–114 年分科測驗歷屆，含非選與選填。',
    /* elective：分科測驗是分科報考，考生只考自己選的那幾科，故與學測同樣提供勾選。 */
    elective: true,
    subjects: ['數學甲', '數學乙', '物理', '化學', '生物', '歷史', '地理', '公民與社會'],
    notes: {
      '數學甲': '自然組進階數學，含非選計算與選填，重完整推導過程。',
      '數學乙': '社會組數學（114 年新增），偏應用與資料判讀。',
      '物理': '力學、電磁、近代物理，圖表與實驗判讀比重高。',
      '化學': '物質結構、反應速率與平衡、有機化學，含實驗設計。',
      '生物': '細胞與遺傳、生理、生態演化，閱讀題組占比高。',
      '歷史': '臺灣史、中國史、世界史，重史料閱讀與脈絡推論。',
      '地理': '自然地理、人文地理、地理技術，重地圖與統計圖判讀。',
      '公民與社會': '社會、政治、法律、經濟四領域，重情境分析與制度比較。'
    }
  },
  {
...
/* 「沒有字母選項、要自己寫答案」的題型。分科測驗除了非選還有「選填」（填空），
   兩者在作答上是同一件事：一個輸入框，不是四個按鈕。
   🔴 引擎裡原本三處各自寫 `q.type === '非選'` 字面值——分科測驗一進來，
   9 題選填就會掉進選擇題渲染器變成「沒有按鈕的空題」。判準集中在這裡，
   下一個新題型只要改這一行。 */
/* 🔴 名字刻意不叫 isFreeAnswer——grading.js 已有 `isFree(q)`，那是**送分題**
   （官方公告一律給分），與「要自己寫答案」是完全無關的兩件事。兩個名字只差一個字，
   在同一支檔案裡交替出現時極易讀錯。 */
var WRITTEN_ANSWER_TYPES = ['非選', '選填', '申論', '計算'];
function isWrittenAnswer(q) {
  return !!q && WRITTEN_ANSWER_TYPES.indexOf(q.type) >= 0;
}
...
var EXAM_OFFICIAL = {
  ...
  /* 分科測驗（大考中心）作答時間：各科皆 80 分鐘。
     size＝近年一份卷的選擇＋非選代表題數。 */
  'ast': { type: 'mixed', bySubject: {
    '數學甲':     { minutes: 80, size: 11 },
    '數學乙':     { minutes: 80, size: 13 },
    '物理':       { minutes: 80, size: 24 },
    '化學':       { minutes: 80, size: 31 },
    '生物':       { minutes: 80, size: 49 },
    '歷史':       { minutes: 80, size: 44 },
    '地理':       { minutes: 80, size: 41 },
    '公民與社會': { minutes: 80, size: 41 }
  } },
...
var EXAM_GROUPS = [
  { id: 'kaogao', name: '國家考試', blurb: '社工師、律師、會計師、醫師、護理師、諮商與臨床心理師、教師檢定 —— 考選部與教育部歷屆國家考試。' },
