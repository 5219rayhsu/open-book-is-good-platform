...
function renderStatus() {
  ...
  if (bank) {
    ...
    /* `noanswer` 與 `review` 是**兩件事**,不能合併成一個數字:
       待校＝還沒校、校了就能用(是待辦);noanswer＝主管機關從未公布答案(是終局,
       再怎麼查也不會變)。合在一起講會讓人以為那些題總有一天會出現。 */
    var noAnsN = bank.questions.filter(function (q) { return q.parse === 'noanswer'; }).length;
    var why = [];
    if (reviewN) { why.push('另 ' + reviewN + ' 題待校，暫不列入'); }
    if (noAnsN) { why.push('另 ' + noAnsN + ' 題主管機關未公布答案，只能閱讀無法作答'); }
    /* 兩者皆為 0 時不寫「另 0 題…」——那句是解釋落差用的,沒有落差就別佔位。 */
...
/* ===================== 開機 =====================
   ...
   這個串行的代價是一個載入窗口。**2026-08-17 前它是靜默的**:窗口內答完的題,
   五個渲染點的 `if (_ex)` 一起跳過,畫面上與「這題沒有詳解」同形,使用者實際回報了
   (社工師,詳解檔 10.5MB 全站最大,窗口也最長)。當時的理由是「不補到貨重繪要動五個點」
   ——但補的位置根本不在那五個點,而在 explain.js 自己:佔位塊自己記得要被換掉,
   五個呼叫端一行都不用改。**「改動成本高」是估的,估錯了就成了沉默降級的藉口。**
   現況:explEl 在窗口內回「載入中」佔位,詳解落地時就地替換(explain.js `_explPending`)。
   sw.js 的 SWR 讓第二次造訪起直接命中快取,窗口只存在於首次造訪。
   三個 resolver 皆無重試:網路抖動掉了 relations／essays 仍是整個 session 缺席,
   只有詳解那條現在會說出來(EXPL_FAILED)。 */
