...
var CACHE_VERSION = 'v12';  /* v12:完全同題改同場輪替互斥(跨場可練到不同年份的同題)、社工「社會工作管理」115 年起停考標示(模擬考預設不勾)。v11:應考類科個人設定(教師檢定等分組考試可複選類科,過濾進度/雷達/出題/藍圖/模擬選科,reload 生效)。v10:標記折角回歸回顧介面（錯題列表／歷史詳情／儲存題頁），整卷與逐題卡片折角天然沿用；同題幹互斥加通用題幹豁免。v9:儲存題獨立頁、模擬考同題幹互斥、錯題排序與本月窗、標記降級為作答內記號。v8:詳解 justify＋pre-line、模擬考內容去重、題組鏈聚攏與鏈式情境框、標記／儲存、錯題篩選、單題出題演算法升級。v7:封面／選科主題切換 theme-boot.js（預設白底、與內部考科共用 obig:theme 偏好）。v6:預快取改 {cache:'reload'} 繞過 HTTP 快取,修「v5 殼烤進舊 exams.js → 切換跳回原科」。v5=類科兩級選科。v4=教師檢定改名+setExam 導航修正 */
