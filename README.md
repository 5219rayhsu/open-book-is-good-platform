...
radical transparency，而非 attention extraction。本專案不放連勝、紅點、徽章這類為了把人留在畫面前而設計的機制；使用者的時間屬於使用者。每一條備考工作量的推估公式都攤開、可檢驗。作答進度**預設只留在本機瀏覽器**（localStorage）；唯有您主動登入，才會同步到帳號（詳見〈進度與隱私〉）。不做行為追蹤、不投放廣告、不販售資料，也誠實說明：沒有任何系統能保證考試結果。
...
題庫資料可由 [`open-book-is-good-skill`](https://github.com/5219rayhsu/open-book-is-good-skill) 所描述的方法產生；若您想從官方開源試題建立自己的題庫，請參考該 skill。
...
**現在（beta）**：不需要登入即可完整使用。作答進度只存在您的瀏覽器（localStorage），不上傳、沒有第三方分析、沒有追蹤用 cookie。

**建置中**：Google 登入 ＋ Cloudflare R2 雲端備份，用於跨裝置接續與避免進度遺失。它上線時，本節會同步更新，並且明講以下三件事——因為它們都是真的：

1. 您一旦用 Google 登入，Google 就會知道您登入了本站。「無第三方」這句話對已登入的使用者不再成立，本專案不會假裝它還成立。
2. 備份存放在本專案的 R2 儲存桶，維運者技術上讀得到。我們只存讓您接續所需的最小資料——一個帳號識別碼，加上「哪一題答對或答錯、什麼時候」；不存姓名、不記錄 IP、不接任何分析服務。
3. 「別人駭進來也讀不到」與「您忘記密碼時我能幫您救回」，在數學上不可能同時絕對成立：我救得回，就代表我具備解密能力，取得這份能力的攻擊者亦然。beta 選擇可救援，因此**不是**端對端加密。若您不接受這個取捨，**就不要登入**——本機模式的功能完全一樣。

登入與否由您決定。資料主權屬於您，登入功能上線時將一併提供匯出與刪除。

## beta：已知問題

本專案目前仍在建置中，有感於一人力量有限，希冀透過公眾力量一齊完善。
...
衍生物選 CC0，理由與試題本身自由利用同一條：讓題目好用的那段工夫，也應該回到 commons。您不需要問我，也不需要標示我。

## 如何貢獻

### 1. 回報錯誤

學生作答時最擔心的，想必是把錯誤答案當成正解琢磨許久。因此若您在作答時發現：
...
請到對應的 repo 開一個 issue，附上**考試／年度／科目／題號**（或 `qid`），說明哪裡不對，若能附官方出處更好。抓到一個，就替後面每一個讀到這題的人擋掉一次誤導。

### 2. 補寫或訂正詳解

選擇題詳解與申論參考要點目前由 AI 整理，覆蓋率很不平均：

- **完全沒有**：醫師、教師檢定；
- **偏低**：護理師、諮商心理師、臨床心理師；
- **接近全覆蓋**：社工師、律師、會計師、學測、會考。

詳解撰寫方式：對齊官方標準答案、逐選項辨析、引用**現行**法規、對不確定處誠實標註、繁體中文全形標點。這些內容以 CC0 釋出，您補的也一樣回到公眾領域。

若您當前詳解正確無誤，但您有更好的邏輯敘事，也歡迎提出。

### 3. 改進圖表擷取

題目裡的圖表是用程式從官方 PDF 裁切出來的，方法完整寫在 [`open-book-is-good-skill`](https://github.com/5219rayhsu/open-book-is-good-skill)。仍有缺口：教師資格考試約 30 張幾何圖。

學測／會考圖表待重抽，因學測／會考圖表變化較多，圖、表、題目、選項缺少較固定格式，或許需要人工手動截取，希望有人能提供有效擷取方式。

### 4. 引擎與系統

`web/` 底下的統一引擎採 MIT。歡迎修 bug、改善無障礙（a11y）、效能、列印友善版等。前端改動請在本機 `http.server` 驗過、0 console error 再送 PR。

### 5. 一整個新考試（甚至新國家）

這是最大的一種貢獻，做法見上面〈fork 一個新考試〉。您可以 fork 出去長成自己的版本，也可以把新題庫帶回這裡，讓一個網頁同時容下更多考試——這正是我們設想的 plurality。

### 6. 學習工具：專有名詞頁、心智圖

比題庫更上層的兩個學習面，工程較大（都要新的資料結構＋新頁面），先列在此歡迎共建——作者本人也會做，但地基就是要讓任何人都能踩上去。動手前先開 issue 對齊資料結構，免得各長各的：

- **專有名詞定義／清字頁**：把各科的專有名詞、易混淆的字詞與概念，收成一頁可查、可辨析的詞條（詞 → 定義 → 常見誤解 → 關聯題），讓考生查一個詞不必離站，把「這題會」延伸成「這個概念會」。可從既有詳解的「考點」錨與 `relations.json`（概念關聯）長出來。
- **心智圖**：把一科的考點與概念用節點串成一張圖，讓考生看見概念之間的關係、建立整體架構，而不是零散背題。同樣可接 `relations.json` 當骨架；難點在概念抽取與圖的可讀性（節點別爆炸）。

---

小改動直接開 PR；大改動或不確定方向，先開 issue 聊聊（rough consensus, running code）。本專案無意當「最好用的考試 App」，而是一塊任何人都能踩上去、再往外長的地基。
